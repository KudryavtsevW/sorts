...
void decDelenie(sf::RenderWindow &window, std::vector<sf::RectangleShape> &mass, int low, int high, int MAX)
{
  if (low >= high)
  {
    return;
  ...
  decDelenie(window, mass, low, median, MAX);
  decDelenie(window, mass, median + 1, high, MAX);
  decMergeSort(window, mass, low, median, high, MAX);
}
void decMergeSort(sf::RenderWindow &window, std::vector<sf::RectangleShape> &mass, int low, int median, int high, int MAX)
{
